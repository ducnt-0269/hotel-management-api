...
// `templates/booking-request-approval.mjml` in the queue worker, which is why the
// language travels in the context as `i18nLang`.
export function bookingRequestApprovalEmail(
  i18n: I18nService,
  lang: string,
  ...
  const subject = t('subject', { status: t('approval.status') });

  return {
    subject,
    template: 'booking-request-approval.mjml',
    context: {
      i18nLang: lang,
      subject,
      preview: t('approval.body'),
      ...details,
    },
    text: [
      t('greeting'),
