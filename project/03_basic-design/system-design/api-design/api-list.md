...
| 17  | F-010               | POST   | `/booking-requests/:id/cancellation` | User (chủ)  | —                                                                    | Outcome                | Admin → 403; 404 nếu không có hoặc của user khác; 409 `Booking request is not pending` khi không còn `pending` hoặc đã qua `expiresAt` dù cron chưa quét (AC-6 của F-008) |
